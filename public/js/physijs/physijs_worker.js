	...
	switch (description.type) {
		...
		case 'capsule':
			...
			if ((shape = getShapeFromCache(cache_key)) === null) {
				...
				shape = new Ammo.btCapsuleShape(
					description.radius,
					description.height - 2 * description.radius
				);
				setShapeCache(cache_key, shape);
			}
			break;
		...
		case 'heightfield':
			var ptr = Ammo.allocate(
				4 * description.xpts * description.ypts,
				'float',
				Ammo.ALLOC_NORMAL
			);
...
public_functions.removeObject = function (details) {
	...
	if (_noncached_shapes[details.id])
		Ammo.destroy(_noncached_shapes[details.id]);
	var ptr =
		_objects[details.id].a != undefined
			? _objects[details.id].a
			: _objects[details.id].ptr;
...
public_functions.addConstraint = function (details) {
	...
	if (SUPPORT_TRANSFERABLE) {
		constraintreport = new Float32Array(
			1 + _num_constraints * CONSTRAINTREPORT_ITEMSIZE
...
public_functions.simulate = function simulate(params) {
	if (world) {
		...
		params.maxSubSteps =
			params.maxSubSteps || Math.ceil(params.timeStep / fixedTimeStep); // If maxSubSteps is not defined, keep the simulation fully up to date
...
public_functions.hinge_setLimits = function (params) {
	_constraints[params.constraint].setLimit(
		params.low,
		params.high,
		0,
		params.bias_factor,
		...
		if (worldreport.length < 2 + _num_objects * WORLDREPORT_ITEMSIZE) {
			worldreport = new Float32Array(
				2 + // message id & # objects in report
					Math.ceil(_num_objects / REPORT_CHUNKSIZE) *
						REPORT_CHUNKSIZE *
...
reportVehicles = function () {
	...
	if (SUPPORT_TRANSFERABLE) {
		if (vehiclereport.length < 2 + _num_wheels * VEHICLEREPORT_ITEMSIZE) {
			vehiclereport = new Float32Array(
				2 + // message id & # objects in report
					Math.ceil(_num_wheels / REPORT_CHUNKSIZE) *
						REPORT_CHUNKSIZE *
...
reportConstraints = function () {
	...
	if (SUPPORT_TRANSFERABLE) {
		if (
			constraintreport.length <
			2 + _num_constraints * CONSTRAINTREPORT_ITEMSIZE
		) {
			constraintreport = new Float32Array(
				2 + // message id & # objects in report
					Math.ceil(_num_constraints / REPORT_CHUNKSIZE) *
						REPORT_CHUNKSIZE *
