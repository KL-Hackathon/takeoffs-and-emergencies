window.Physijs = (function () {
				...
				for (j = 0; j < collisions[id1].length; j++) {
					...
					if (object2) {
						...
						if (object._physijs.touches.indexOf(id2) === -1) {
							...
							var normal_offset =
								normal_offsets[object._physijs.id + '-' + object2._physijs.id];
							if (normal_offset > 0) {
								_temp_vector3_1.set(
									-data[normal_offset],
	...
	Physijs.Scene.prototype.remove = function (object) {
		...
		if (
			...
			this._materials_ref_counts.hasOwnProperty(object.material._physijs.id)
		) {
			this._materials_ref_counts[object.material._physijs.id]--;
	...
	Physijs.HeightfieldMesh = function (geometry, material, mass, xdiv, ydiv) {
		...
		for (var i = 0; i < geometry.vertices.length; i++) {
			...
			b = Math.round(
				i / this._physijs.xpts - (i % this._physijs.xpts) / this._physijs.xpts
			);
			points[i] =
				geometry.vertices[
	...
	Physijs.VehicleTuning = function (
		...
		this.suspension_compression =
			suspension_compression !== undefined ? suspension_compression : 0.83;
